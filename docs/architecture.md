...
- 作成時の `inquiry`、`report`、`customer_visit` はそれぞれ問い合わせ、報告書、客先同行トラッカーへ解決する。報告書渡し済みは `report`、予定・担当者アサイン済みは `customer_visit` でのみサポート担当者に公開する。客先同行では同行方法を必須入力とし、作成者には読み取り専用、サポート担当者には更新可能な値として返す。
