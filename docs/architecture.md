...
- 優先度の選択肢は Redmine の列挙値を正とし、作成・詳細画面で共通利用する。
- 作成時の `inquiry`、`report`、`customer_visit` はそれぞれ問い合わせ、報告書、客先同行トラッカーへ解決する。報告書渡し済みは `report`、予定・担当者アサイン済みは `customer_visit` でのみサポート担当者に公開する。客先同行では同行方法を必須入力とし、開始希望日時2件と予定会議時間は任意入力とする。営業担当者は作成時だけ訪問希望情報を設定でき、以後は読み取り専用、サポート担当者は更新可能とする。
