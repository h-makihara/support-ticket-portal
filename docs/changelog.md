...
- 任意の「開始希望日時 第一希望」「開始希望日時 第二希望」「予定会議時間（分）」を追加
- 日時を `YYYY-MM-DD hh:mm` 形式の実在日時、会議時間を1分以上の整数としてAPIで検証
- 営業担当者は作成時のみ入力可能、作成後は読み取り専用、サポート担当者は変更可能

### 客先同行の同行方法

- 客先同行トラッカー専用の必須カスタムフィールド「同行方法」を追加
- 作成時にオンライン／オフラインを選択し、作成後は営業担当者へ読み取り専用で表示
- サポート担当者はチケット詳細のドロップダウンから同行方法を変更可能

## 2026-08-21

### Tracker-based request types

- 問い合わせ、報告書、客先同行を相互排他的なトラッカーとして作成・表示するワークフローへ移行
- 報告書渡し済みと予定・担当者アサイン済みを該当トラッカーだけの完了項目へ変更し、両方が必要な依頼は別チケットで管理
- 旧要否フィールドの移行時に既存プロジェクトチケットを一度だけ削除し、旧フィールドを削除
- トラッカーID環境変数を廃止し、トラッカー解決を実行時設定へ統一
- Playwright の作成・観点別・フルリグレッションをトラッカー選択と個別チケットフローへ更新
