...
客先同行には任意の「開始希望日時 第一希望」「開始希望日時 第二希望」「予定会議時間」もあります。Redmine 6.1標準には日時型がないため、開始希望日時は文字列型で `YYYY-MM-DD hh:mm` 形式、予定会議時間は整数型で分数を保存します。営業担当者は作成時のみ入力でき、作成後は読み取り専用です。サポート担当者はPortalの対応情報から変更できます。

同行方法と訪問希望情報の作成後の更新権限はPortal APIで制御します。営業担当者がRedmineの画面やREST APIからチケットを直接変更する運用はサポート対象外です。
...
対応情報の完了項目はサポート担当者だけが更新できます。「報告書を渡した」は報告書トラッカーにだけ、「予定・担当者をアサインした」「同行方法」「開始希望日時」「予定会議時間」は客先同行トラッカーにだけ表示・更新できます。完了項目を有効にすると「対応済」に変更し、担当者をチケット作成者へ戻します。
